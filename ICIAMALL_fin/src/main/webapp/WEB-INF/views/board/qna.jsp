...
<body>
	<div id="qna_all">
			...
			<table id="qna_table">
				...
				<tbody id="list">
					<c:forEach items="${page.list }" var="b">
						<tr>
							<td width="5%" id="qna_bno_td">${b.bno }</td>
							<td width="77%" id="qna_title_td"><a href="/www/board/qnaread?bno=${b.bno }">${b.title }&nbsp;[${b.commentCnt }]</a></td>	
							<td width="6%" id="qna_writer_td">${b.writer }</td>
							<td width="7%">${b.writeTimeString }</td>
							<td width="5%">${b.readCnt }</td>
						</tr>
					</c:forEach>
				</tbody>
		...
		<div id="qna_write_div">
			<a href="/www/board/qna_write" id="qna_write_btn">WRITE</a>
		</div>
		</sec:authorize>
		...
		<div id="pagination" style="text-align: center;">
			<ul class="pagination">
				<c:if test="${page.prev>0 }">
					<li id="prev_btn"><a href="/www/board/qna?pageno=${page.prev }">이전으로</a></li>
				</c:if>
				
				<c:forEach begin="${page.start }" end="${page.end }" var="i">		<!-- c:forEach 반복문으로 시작번호부터 끝번호까지 출력 -->
					<c:if test="${page.pageno==i}">
						<li id="page_active"><a href="/www/board/qna?pageno=${i }">${i}</a></li>
					</c:if>
					<c:if test="${page.pageno != i }">
						<li id="page_nonactive"><a href="/www/board/qna?pageno=${i }">${i}</a></li>
					</c:if>
				</c:forEach>
				
				<c:if test="${page.next>0 }">
					<li><a href="/www/board/qna?pageno=${page.next }">다음으로</a></li>
				</c:if>
			</ul>
		</div>
